...
export REF_FILE=$(find work_singularity_profile | grep -v scheme | grep 'ref.fa$' | head -n1 | xargs readlink | sed s'/\/work\//\/work_singularity_profile\//'g)
export BED_FILE=$(find work_singularity_profile | grep -v scheme | grep 'bed$' | head -n1 | xargs readlink | sed s'/\/work\//\/work_singularity_profile\//'g)
